...
# GCP FortiGate Terraform Deployment
...
* Network Diagram
    ...
    ![diagram1](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/network-diagram.png)


***

***All work in this lab will take place in Project ONE***
...
* This lab will use Project One.  Click on **Open Console** in the **Student Resources** area to the left of the screen. 

* This will open a new tab in your browser on your sign in page and pre-populate the User Account information.  Click **Next**

    ![console2](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/console-login-1.png)

* Input the previously noted password

    ![console3](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/console-login-2.png)
...
* Accept all popups and warnings.  You are now at your Console Home screen.  Not the Pinned products down the left side of the screen.
    ...
    ![console4](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/console-home.png)
...
* On the left pane, click on **VPC network**
    ...
    ![console5](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/VPC-Network-left-pane.png)
...
1. Under **Network interface**, click the down arrow to expand **default** and change the network settings as follows.  Note that  we are **NOT** assigning an External IP address for this instance.
    ...
    ![console17](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/ubuntu-nic.png)
...
* In the center of the screen, click  on **ROUTES**.  In the Region dropdown, choose **us-central1 (iowa)** and then click **Route management**.
    ...
    ![console19](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/new-route-add.png)

* This will take you to the VPC network > Routes Screen.  Click on **CREATE ROUTE**

    ![console25](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/create-route.png)
...
1. Click **OK** to continue
    ...
    ![console22](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/fortigate-vip-http.png)
...
***All terraform needs to be run in Project Two for this lab***

**Reminder to change project int cli use**
...
10. Move to **Project 2**
11. Repeat the above steps 1-4 for **Project 2**
12. Select the vpc networked named **fgt-qlabs-internal-vpc**
13. Under **Peered VPC network** select **In another project** and enter the **Project 1** Project ID and webserver vpc network name **qwiklabs-webserver-public-vpc-"xxx"**
...
* Network Diagram
    ...
    ![diagram1](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/full-network-diagram.png)
...
* From the GCP console dashboard, select click on the cursor **>_** at the top of the screen.  This will open a **CLOUD SHELL Terminal** at the bottom of the screen.
    ...
    ![overlay1](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/open-shell.png)

* Get the project-id by copying it from the cloud shell prompt.  We do not need the open and close parentheses.

    ![overlay2](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/get-project.png)
...
* Get the lb-ip.  Under hamburger menu  select MORE PRODUCTS > Network services > **Load balancing**.  In the center of the screen, Click on LOAD BALANCERS > **fgt-qlabs-bes-elb-us-central1**.
    ...
    ![overlay3](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/load-balancing.png)

* In the center of the screen, under **Frontend** copy the IP address

    ![overlay4](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/frontend-ip.png)
...
* Once you have the project-id and lb-ip, update the sdk command from earlier and input it into the cloud shell.  Below is an example of what that command should look like.
    ...
    ![overlay5](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/sdk-sample.png)
...
* Log into the active FortiGate of the cloud on-ramp HA pair.  On the left pane, select **Network** > **Inerfaces**.  Click on port1 and select **Edit**  Under Address, toggle the **Secondary IP address** button and input the lb-ip from earlier.
    ...
    ![overlay6](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/secondary-ip.png)
...
### Task 5 - Configure BGP on remote FortiGate
...
* Below are the expected outputs
    ...
    ![overlay7](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/hub-bgp-sum.png)

    ![overlay8](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/spoke-bgp-sum.png)    
...
* Navigate to **Network > SD-WAN** and click on **Create New > SD-WAN Member** From the **Interface** drop down, choose **port1**.  Leave all other values as default.
    ...
    ![overlay9](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/new-sdwan-member.png)
...
* Navigate to **Network > SD-WAN** and click on **Create New > SD-WAN Member** From the **Interface** drop down, choose **HUB1**.  In the **SD-WAN Zone** drop down, click **Create** and name the new zone "overlay".  Leave all other values as default and click **OK** 
    ...
    ![overlay10](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/hub1-sdwan.png)
...
* Navigate to **Network > SD-WAN > Performance SLAs** and select the test named **Default_Google_Search"**.  Click **Edit**. Under **Participants** select **All SD-WAN Members**.  Leave all other values as default and click **OK**.  
    ...
    ![overlay11](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/google-sla.png)
...
* You may need to refresh the browser in order to see the SLA measurements.  Click on **Default_Google Search**.  You should now see performance data updating in real time for both the **HUB1** and **port1** interfaces.
    ...
    ![overlay12](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/google-mon.png)
...
* In the first two steps, we used the default Google performance SLA monitor.  While it's not unheard of to monitor a Public internet site over an IPSec tunnel to the cloud, a more realistic scenario would be to monitor a resource in our own cloud "Data Center"  Below is an example of a custom performance SLA monitoring hour Hub Ubunt Server (created in lab 3).
    ...
    ![overlay13](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/ubu-hub-mon.png)
...
* Navigate to **Network > SD-WAN > SD-WAN Rules**.  Click **Create new**  Feel free to play around with the Values here.  At a minimum you will need to provide **Name**, **Destination Address or Internet Service**, **Interface selection strategy** and **Interface and/or Zone preference**.  **Note: The minimum required information will change, depending on which selection strategy you choose.  Our example below uses Best Quality, which additionally, requires us to choos a Measured SLA and Quality Criteria**
    ...
    ![overlay14](https://raw.githubusercontent.com/fortidg/markdown-test/main/images/oci-rule.png)
